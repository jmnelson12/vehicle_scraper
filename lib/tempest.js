...
const callToAutoTempest = async function(params) {
	let { make, model, zip, radius } = params;

	make = make === "any" ? "" : make;
	model = model === "any" ? "" : model;
	radius = radius === "any" ? -1 : radius;

	const base_url = `https://www.autotempest.com/queue-results?make=${make}&model=${model}&radius=${radius}&originalradius=${radius}&sort=price_asc&saleby=any&simplified=0&transmission=any&zip=${zip}&sites=`;
